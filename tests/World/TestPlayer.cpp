...
TEST(Player, ConstructionSetsIdentityAndPreChargesFood) {
  /*
   * Given a brand new player at known coordinates
   * When the constructor completes
   * Then identity, position, direction, level 1 and food reserve are set
   */
...
TEST(Player, MutatorsUpdateStateInPlace) {
  /*
   * Given a player at the origin facing North
   * When setPosition, setDirection and setLevel are called
   * Then the corresponding fields take the new values
   */
...
TEST(Direction, LetterMatchesProtocolMnemonics) {
   ...
   * When directionLetter is queried for each
   * Then the GUI protocol mnemonics N/E/S/O come back
   */
