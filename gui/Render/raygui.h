...
typedef enum {
  ...
  TEXT_ALIGNMENT,  // Control text horizontal alignment inside control text
...
RAYGUIAPI void GuiSetAlpha(
    float alpha);  // Set gui controls alpha (global state), alpha goes from
                   // 0.0f to 1.0f
...
RAYGUIAPI char** GuiLoadIcons(
    const char* fileName, bool loadIconsName);  // Load raygui icons file (.rgi)
                                                // into internal icons data
...
RAYGUIAPI int GuiGrid(
    Rectangle bounds, const char* text, float spacing, int subdivs,
...
RAYGUIAPI int GuiTextInputBox(
    ...
    bool* secretViewActive);  // Text Input Box control, ask for text, supports
                              // secret
...
RAYGUIAPI int GuiColorPickerHSV(
    Rectangle bounds, const char* text,
    Vector3* colorHsv);  // Color Picker control that avoids conversion to RGB
                         // on each call (multiple color controls)
RAYGUIAPI int GuiColorPanelHSV(
    Rectangle bounds, const char* text,
    Vector3* colorHsv);  // Color Panel control that returns HSV color value,
...
static Texture2D LoadTextureFromImage(
    Image image);  // -- GuiLoadStyle(), required to load texture from embedded
                   // font atlas image
static void SetShapesTexture(
    Texture2D tex, Rectangle rec);  // -- GuiLoadStyle(), required to set shapes
...
static const char* GetDirectoryPath(
    const char* filePath);  // -- GuiLoadStyle(), required to find charset/font
                            // file from text .rgs
...
static int GetTextWidth(
    const char* text);  // Gui get text width using gui font and style
...
static const char** GuiTextSplit(
    const char* text, char delimiter, int* count,
...
static void GuiTooltip(
    Rectangle controlRec);  // Draw tooltip using control rec position

static Color GuiFade(Color color,
                     float alpha);  // Fade color by an alpha factor
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue,
  ...
  GuiDrawRectangle(
      ...
      GetColor(GuiGetStyle(LISTVIEW, BORDER + state * 3)),
      GetColor(
          GuiGetStyle(DEFAULT, BORDER_COLOR_DISABLED)));  // Draw the background
  ...
  if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE)) {
    ...
    GuiDrawText(
        ...
        GetColor(GuiGetStyle(
            SCROLLBAR,
            TEXT +
                state * 3)));  // ICON_ARROW_DOWN_FILL / ICON_ARROW_RIGHT_FILL
#endif
  }
  //--------------------------------------------------------------------
